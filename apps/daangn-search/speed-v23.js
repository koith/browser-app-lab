// v2.3 speed-first scheduler: restore the old large-batch feel.
// Search 15 dongs per API call, two API calls at a time, no fixed sleeps.
// Blocked/failed dongs are returned immediately and can be retried manually.
const SPEED_BATCH=15, SPEED_PARALLEL=2;
...
searchAll=async function(opts={}){
  ...
  try{
    ...
    save('dgn_diag_v23',{at:Date.now(),mode,q,...lastMeta,failedCodes});
...
$('limit').textContent='동네 최대 200곳 · 속도 우선 검색';
